...
def get_data(conditions, filters):
	...
	data = frappe.db.sql(
		...
		SELECT
			...
			so.status, so.customer, so.payment_terms_template, so.po_no, so.project, soi.item_code,
			...
			poi.qty as ord_qty,
			poi.received_qty as rec_qty,
			dni.returned_qty as damaged_qty,
			dni.installed_qty as inst_qty
		FROM
			`tabSales Order` so,
		...
		LEFT JOIN `tabPurchase Order Item` poi
			ON poi.sales_order_item = soi.name and poi.docstatus =1
